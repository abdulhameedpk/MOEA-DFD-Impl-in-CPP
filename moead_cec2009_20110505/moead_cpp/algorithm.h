...
//	double  rate;          //  the differential rate between solutions
	...
	int     gen;           // gen made global for calc tau in MOEADFD algorithm
...
/*==========================================================================
//  C++ Implementation of MOEA/DFD presented in "An Improved Multiobjective Evolutionary Algorithm based on Decomposition with Fuzzy Dominance
 * " by Md Nasir et al.
 *
 */
...
public:
      ...
      int A ;      // fuzzy function decay constant
		...
		// to i-dominance level
			...
			double prod=1;
			 ...
			 for (int j = 0; j < perm.size(); j++) { 					// j \in P
				 ...
				 CIndividual individ=population[perm[j]].indiv;
				 prod=1;	//reset prod
...
void CMOEADFD::exec_emo(int run)
{
    char filename[1024];
...
//	sprintf(filename,"POFS/PS_%s_%d.dat",strTestInstance,run);
//	save_pos(filename);
	sprintf(filename,"POF/PF_DFD_%s_%d.dat",strTestInstance, run);
	save_front(filename);
